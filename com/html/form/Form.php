 ...
 * @todo implement countable (an SPL interface)
 * @todo possibly change $this->fields to be of class SplObjectStorage so that it doesn't have to be a nested array.
...
class Form extends HTMLElement implements Iterator, ArrayAccess{
	...
	/**
	 * Internal. Used to see if a call to setEnctype() has been made. (see method addField())
	 */ 
	...
	*/
	...
	/**
	 * Prints the HTML for the form. Has alias __toString().
	 ...
	 * @param int $maxLabelWidth (Optional) The largest width a label can have (units are 'ex'). Only used when $printKevStyle is true.
	 * @return string A string representation of this form. ie. The HTML
	 * @todo Simplify the code for this hideous (but working) method
	 */
	public function render($printKevStyle=true, $breakString= "\n<br />", $maxLabelWidth=100){
		$this->addAttribute("enctype", $this->enctype);
		$inner = "\n";
		$longestLabelLength = 0;
		foreach($this->fields as $key => $field) {	//Find length of longest label.
		    if(is_array($field)){	//this means we have a whole bunch of radio buttons w/ the same name attribute
		    	foreach($field as $key2 => $radio){
		    		if(strlen($radio->getLabelString()) > $longestLabelLength && strlen($radio->getLabelString()) < $maxLabelWidth){
						$longestLabelLength = strlen($radio->getLabelString());
					}
		    	}
		    }else{
			    if(strlen($field->getLabelString()) > $longestLabelLength && strlen($field->getLabelString()) < $maxLabelWidth){
					$longestLabelLength = strlen($field->getLabelString());
				}
			}
		}
		if($this->printFields){
			foreach($this->fields as $key => $field){
				if(is_array($field)){	//this means we have a whole bunch of radio buttons w/ the same name attribute
					foreach($field as $value => $Element){
						if($printKevStyle){
							$inner .= $this->kevStyleHelper($Element, $longestLabelLength);
						}else{
					...
					if($printKevStyle){
						$inner .= $this->kevStyleHelper($field, $longestLabelLength);
	...
	/**
	 * Internal method. A helper function for render().
	 */
	private function kevStyleHelper($field, $width){
		$field->setPrintLabel(false);
		$inner = "<span class= \"label\" style= \"width: {$width}ex;display: inline-block;text-align: right;margin-right: 10px;vertical-align: top;\">" . $field->getLabelElement() . "</span>";
